...
class Basemap(GeoEngine):
    ...
    def render_vis_grad(self, vehicle_list, ego_vid, current_t_idx=None, grad_res={}, with_traj=True, linewidth=2, color_vid_list=None):

        # self.basemap = cv2.cvtColor(self.basemap, cv2.COLOR_RGB2RGBA)
        # self.basemap[:, :, 3] = 1

        base_layer = self.draw_location_vis_grad(vehicle_list, current_t_idx=current_t_idx, ego_vid=ego_vid, grad_res=grad_res, color_vid_list=color_vid_list)
        if with_traj:
            traj_layer, traj_alpha = self.draw_trajectory(vehicle_list, linewidth)
            map_vis = self.layer_blending(base_layer, traj_layer, traj_alpha)
            map_vis = (map_vis*255.).astype(np.uint8)
        else:
            map_vis = base_layer
    ...
    # def get_pixel_resolution(self):
    #     # the height/width scale should be the same with the original figure
    #     assert np.abs(self.w/self.h - 1678/936) < 1e-2
    #
    #     # For rounD dataset
    #     self.x_pixel_per_meter = (1 / (0.0101601513616588 * 10.)) * self.w / 1678
    #
    #     self.y_pixel_per_meter = 1 / (0.0101601513616588 * 10.) * self.h / 936


class Basemap_highlight_yielding_behavior(Basemap):

    def render(self, vehicle_list, with_traj=True, linewidth=2, traj_df=None):

        base_layer = self.draw_location(vehicle_list, traj_df)
        if with_traj:
            traj_layer, traj_alpha = self.draw_trajectory(vehicle_list, linewidth)
            map_vis = self.layer_blending(base_layer, traj_layer, traj_alpha)
            map_vis = (map_vis*255.).astype(np.uint8)
    ...
    def draw_trajectory(self, vehicle_list, linewidth=2):
    ...
    def draw_location(self, vehicle_list, traj_df=None):
        ...
        for i in range(0, len(vehicle_list)):
            ...
            is_yielding_flag = traj_df[traj_df['vid'] == v.id]['is_yielding'].item()
            if is_yielding_flag is None:
                color = [192, 192, 192]  # grey
            if is_yielding_flag is False:
                color = [147, 183, 89]  # green
            if is_yielding_flag is True:
                color = [255, 0, 0]  # red
