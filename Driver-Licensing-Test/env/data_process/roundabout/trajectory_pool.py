...
class TrajectoryPool(object):
    ...
    def update(self, vehicle_list, ROI_matching=False):
        ...
        for i in range(len(vehicle_list)):
            ...
            if v.location.x == None or v.location.y == None:
                continue
            # pxl_pt = self.road_matcher._world2pxl([v.location.x, v.location.y])  # When analyze gt metrics, we may want to remove exit vehicle, then comment back these 4 lines.
    ...
    def get_vehicles_by_id(self, id):
        return self.pool[id]
