...
class VRUAtCrosswalkGen:
    def __init__(self, settings):
        self.scenario = 'VRU Crossing the Street at the Crosswalk'
    ...
    def init(self):
        feasibility_data_file = self.risk_level_path + '/' + self.name + '.npz'
        flag = False
        if not os.path.exists(feasibility_data_file):
    ...
    def risk_level_gen(self):
        # L_min is from the AV starting point to the start of VRU Crossing the Street at the Crosswalk point of BV
        L_min = np.zeros((len(self.dec_bound),len(self.v_VRU_pool),len(self.v_AV_pool)))

        # Assume the AV notice when VRU start to cross the road
        for i in range(len(self.dec_bound)):
            for j in range(len(self.v_VRU_pool)):
                t0 = (self.lateral_dis-self.lane_width/2)/self.v_VRU_pool[j]  # moment when VRU enter the lane
                t1 = (self.lateral_dis-self.AV_width/2)/self.v_VRU_pool[j]  # moment when VRU enter the conflict zone
                t2 = (self.lateral_dis+self.AV_width/2)/self.v_VRU_pool[j]  # moment when VRU exit the conflict zone
                for k in range(len(self.v_AV_pool)):
                    if t2 - t0 < self.t_reac[i]:  # AV cannot notice when VRU enter the lane
                        L_min[i,j,k] = self.v_AV_pool[k]*t2
                    else:
                        if self.v_AV_pool[k] < -self.dec_bound[i]*(t2-t0-self.t_reac[i]):  # AV speed is lower, AV can stop before entering conflict zone
                            L_min[i,j,k] = self.v_AV_pool[k]*(self.t_reac[i]+t0) - (self.v_AV_pool[k]**2)*0.5/self.dec_bound[i]  # + self.AV_length / 2
                        else:  # AV speed is higher, cannot stop when VRU enters the conflict zone
                            L_min[i,j,k] = self.v_AV_pool[k]*t2 + 0.5*self.dec_bound[i]*(t2-t0-self.t_reac[i])**2  # + self.AV_length / 2
    ...
    def generate_risk_level_bounds(self):
        risk_level_bounds = self.feasibility_path + '/' + self.name + '.npz'
        try:
            self.L_min = np.load(risk_level_bounds)
    ...
    def generate_cases(self):
        if len(self.risk_level_bounds) == 0:
            print(colored(f'[ {self.scenario} ] Risk level bounds have not been generated!', 'red'))
            self.generate_risk_level_bounds()

        print(f'[ {self.scenario} ] Generating VRU Crossing the Street at the Crosswalk cases...')
        ...
        for risk_level, cases in generated_cases.items():
            self.cases[risk_level] = [
                {
                    'dis to edge': self.lateral_dis - self.lane_width / 2,
                    'alongside sp': 0,
                    'triggered dis': dis,
                    'crossing sp': sp,
                    'crossing dis': 6
