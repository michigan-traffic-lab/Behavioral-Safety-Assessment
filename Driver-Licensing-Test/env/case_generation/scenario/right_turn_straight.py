...
from env.case_generation.scenario.right_turn_straight_feasibility_analysis import *
from env.case_generation.scenario.right_turn_feasibility_plot import *


class RightTurnStraightGen:
    def __init__(self, settings):
        self.scenario = 'Right Turn (AV goes straight)'
    ...
    def init(self):
        ...
        if not os.path.exists(cases_data_file):
            ...
            if self.v_AV != self.cases['v_AV']:
                print(colored(f'[ {self.scenario} ] Case speed is wrong! Expected ' + str(self.v_AV) + ' but now is ' + str(self.cases['self.v_AV']) + ' exist.', 'red'))
                flag = True
    ...
    def choose_case(self, round_num=-1):
        for risk_level, cases in self.cases.items():
            if round_num < len(cases):
                ...
                real_case = {
                    'dis': selected_case['dis'] + self.turning_radius - self.AV_length / 2,  # distance to the conflict point alongside the track
                    'sp': selected_case['sp']
                }
