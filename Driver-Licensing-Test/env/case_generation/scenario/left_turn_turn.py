...
class LeftTurnTurnGen:
    def __init__(self, settings):
        self.scenario = 'Left Turn (AV turns left)'
        ...
        alpha = math.acos((self.turning_radius - self.lane_width + self.BV_width / 2) / (self.turning_radius - self.AV_width / 2))
        beta = math.atan((self.AV_length / 2) / (self.turning_radius - self.AV_width / 2))
        self.arc_len_1 = (alpha - beta) * (self.turning_radius - self.AV_width / 2)  # distance for AV enter conflict area
        d1 = math.sin(alpha) * (self.turning_radius - self.AV_width / 2)  # dist between AV starting center pt and the upper bound of conflict zone
        gamma = math.acos((self.turning_radius - self.lane_width - self.AV_width / 2) / (self.turning_radius + self.AV_width / 2))
        theta = math.atan((self.AV_length / 2) / (self.turning_radius + self.AV_width / 2))
        self.arc_len_2 = (gamma + theta) * (self.turning_radius + self.AV_width / 2)  # distance for AV leave conflict area
        d2 = math.sin(gamma) * (self.turning_radius + self.AV_width / 2) - d1  # dist between lower & upper bound of conflict zone
        self.L_intersection = settings.scenario_info[self.scenario]['intersection length']  # distance from the stop line to another stop line
        d1 += settings.scenario_info[self.scenario]['turning offset']
        self.L4 = self.L_intersection - d1 - d2  # distance from the stop line on the straight track to the lower bound of conflict zone
        self.L5 = self.L_intersection - d1  # distance from the stop line on the straight track to the upper bound of conflict zone
        ...
        self.dx_min = self.L_intersection - self.turning_offset
        self.step = 0.1
    ...
    def risk_level_gen(self):
        ...
        # Assume the AV notice when BV enters the intersection area
        for i in range(len(amin)):
            for j in range(len(v_BV_pool)):
                t1 = self.L4 / v_BV_pool[j]  # moment when BV head enters the conflict zone
                t2 = (self.L5 + self.BV_length)/v_BV_pool[j]  # moment when BV tail exits the conflict zone
                for k in range(len(v_AV_pool)):
                    if t2-t_reac_dec[i] > 0 and v_AV_pool[k] < -amin[i]*max(t2-t_reac_dec[i], 0):  # AV speed is lower, AV can stop before entering conflict zone
                        L_min_dec[i,j,k] = v_AV_pool[k]*t_reac_dec[i] - (v_AV_pool[k]**2)*0.5/amin[i] + self.L_intersection - self.arc_len_1 - self.turning_offset - self.AV_length / 2
                    else:  # AV speed is higher, cannot stop when enter the conflict zone
                        L_min_dec[i,j,k] = v_AV_pool[k]*t2 + 0.5*amin[i]*max(t2-t_reac_dec[i], 0)**2 + self.L_intersection - self.arc_len_1 - self.turning_offset - self.AV_length / 2
                    # AV can pass the conflict zone ahead of BV
                    L_max_acc[i,j,k] = v_AV_pool[k]*t1 + 0.5*amax[i]*max(t1-t_reac_acc[i], 0)**2 + self.L_intersection - self.arc_len_2 - self.AV_length / 2 - self.turning_offset
    ...
    def choose_case(self, round_num=-1):
        for risk_level, cases in self.cases.items():
            if round_num < len(cases):
                ...
                real_case = {
                    'dis': selected_case['dis'] - self.L_intersection + self.turning_offset + math.acos(1 - self.lane_width / self.turning_radius) * self.turning_radius,  # distance to the conflict point alongside the track
                    'sp': selected_case['sp']
                }
