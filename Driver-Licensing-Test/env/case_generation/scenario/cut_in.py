...
class CutInGen:
    ...
    def risk_level_gen(self):
        ...
        for i in range(len(self.amin)):
            for j in range(len(self.dv)):
                dx[j,i] = -self.dv[j]**2 /(2*self.amin[i]) + (self.t_reac[i]+t1)*self.dv[j]  # then dx_min is what it takes to make self.dv to zero
    ...
    def generate_cases(self):
        ...
        generated_cases = self.covering_suite.k_wise_covering_suite(self.risk_levels, self.risk_level_bounds, self.dv, self.v_res, self.d_res)
