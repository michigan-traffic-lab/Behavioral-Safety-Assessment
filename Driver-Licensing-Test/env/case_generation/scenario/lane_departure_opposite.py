...
class LaneDepartureOppositeGen:
    def __init__(self, settings):
        self.scenario = 'Lane Departure (opposite direction)'
        ...
        self.lc_time = settings.scenario_info[self.scenario]['lane change duration']
    ...
    def risk_level_gen(self):
        dx = np.zeros((len(self.dv),len(self.lc_time))) 
        TTC = np.zeros((len(self.dv),len(self.lc_time)))
        for i in range(len(self.lc_time)):
            for j in range(len(self.dv)):
                dx[j,i] = self.dv[j] * (self.lc_time[i] + self.t_reac[i])
                if self.dv[j]<=0:
                    TTC[j,i] = 100000
                else:
